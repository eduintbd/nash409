...
export const OwnerPropertyCard = ({
  ...
  const occupancyRate = totalFlats > 0 ? Math.round(((rentedFlats + selfOccupied) / totalFlats) * 100) : 0;
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardDescription className="flex items-center gap-2">
            ...
            {language === 'bn' ? 'আমার ফ্ল্যাট' : 'My Properties'}
          </CardDescription>
          <span className="text-2xl font-bold text-primary">{totalFlats}</span>
        </div>
        <CardTitle className="text-sm text-muted-foreground">
          {language === 'bn' ? 'মোট ফ্ল্যাট' : 'Total Flats'}
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3 pt-2">
          <div className="text-center p-2 rounded-lg bg-success/10">
            ...
            <p className="text-lg font-bold text-success">{rentedFlats}</p>
          ...
          <div className="text-center p-2 rounded-lg bg-primary/10">
            ...
            <p className="text-lg font-bold text-primary">{selfOccupied}</p>
            <p className="text-[10px] text-muted-foreground leading-tight">
              {language === 'bn' ? 'নিজে' : 'Self'}
            </p>
          </div>
